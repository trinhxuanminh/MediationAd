...
enum Event {
  ...
  case consentManagerChange(ConsentManager.State)
  ...
  case adLoadSuccess(MonetizationNetwork, AdManager.AdType, String?)
  ...
  var name: String {
    ...
    case .remoteManagerLoadFail:
      return "RemoteManager_Load_Fail"
    case .remoteManagerTimeout:
    ...
    case .adLoadSuccess(let monetizationNetwork, _, _):
  ...
  var parameters: [String: Any]? {
    switch self {
    case .releaseManagerChange(let state):
      return ["state": state.rawValue]
    case .consentManagerChange(let state):
      return ["state": state.rawValue]
    ...
    case .adLoadTimeout(_, let adType, let adUnitID):
      return [LogEventManager.adType(adType): adUnitID ?? String()]
    case .adLoadSuccess(_, let adType, let adUnitID):
      return [LogEventManager.adType(adType): adUnitID ?? String()]
